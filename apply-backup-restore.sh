...
verify_dash() {
  local f="$1"
  local ok=1
  for needle in "doPrepperBackup" "confirmPrepperRestore" "bakRestorePanel" "/api/backup" "/* bakRestore */" "bakLabel"; do
  ...
  if grep -qF "window.prompt" "$f"; then
    echo "FAIL verify: window.prompt still present (mobile-unfriendly)"
    ok=0
  else
    echo "OK verify: no window.prompt"
  fi
