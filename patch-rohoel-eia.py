...
# Replace whatever fetch_rohoel() body is present (exact EuroOilWatch text may differ on Pi)
start = src.find("def fetch_rohoel():")
if start < 0:
    raise SystemExit("STOP fetch_rohoel: def fetch_rohoel() nicht gefunden. Backup bleibt: " + str(bak))
end = -1
end_len = 0
for marker in ("\n    return result\n", "\n\treturn result\n", "\r\n    return result\r\n"):
    pos = src.find(marker, start)
    if pos >= 0 and (end < 0 or pos < end):
        end, end_len = pos, len(marker)
if end < 0:
    raise SystemExit("STOP fetch_rohoel: return result nicht gefunden. Backup bleibt: " + str(bak) + " Kopf: " + repr(src[start:start+160]))
end = end + end_len
# Normalize CRLF in file after read? Path read_text already normalizes? keep as-is

# Guard: replaced span must look like the oil fetcher
span = src[start:end]
low = span.lower()
if not any(x in low for x in ("eurooilwatch", "brent", "rohoel", "oil_history", "priceeur")):
    raise SystemExit("STOP fetch_rohoel: unerwarteter Funktionsinhalt. Backup bleibt: " + str(bak) + " Kopf: " + repr(span[:120]))
print("fetch_rohoel span bytes:", len(span))
