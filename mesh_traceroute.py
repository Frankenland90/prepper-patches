...
def consecutive_fails(path: Path | None = None) -> int:
    """Count trailing ok=false samples from scheduled probes only.

    Manual button probes (manual=True) are ignored for Funk Ampel —
    only hourly/worker samples count.
    """
    hist = load_hist(path)
    n = 0
...
def execute_probe(
    ...
    *,
    ...
    """Ein Trace über bestehende iface-Session. sendData unter lock. # meshTraceManual

    get_iface: zero-arg callable returning current iface (e.g. lambda: _iface).
    lock: bridge _lock — Meshtastic TCP iface needs sendData under this lock;
    wait for response happens outside the lock.
    """
    import threading
    ...
    try:
        ...
        # Proven pattern (071f133): get iface + sendData inside lock; wait outside.
        with lock:
            iface = get_iface()
            if not iface:
                pass
            elif r is not None and hasattr(iface, "sendData"):
                iface.sendData(
                    r,
                    destinationId=dest,
                    portNum=port,
        ...
        if not iface:
            return store_result(
                path, ok=False, dest=dest, error="no iface", manual=manual
            )

        if not done.wait(timeout):
            point = store_result(
                path, ok=False, dest=dest, error="timeout", manual=manual
            )
            log("traceroute: timeout", dest)
            return point
        if box.get("err"):
            point = store_result(
                path, ok=False, dest=dest, error=str(box["err"]), manual=manual
            )
        ...
        if pkt is None and not hasattr(iface, "sendData"):
            point = store_result(
                path, ok=True, dest=dest, error="no-snr-payload", manual=manual
            )
            log("traceroute: ok (kein Payload)", dest)
            return point
        if pkt is None:
            point = store_result(
                path, ok=False, dest=dest, error="no response", manual=manual
            )
        ...
        try:
            point = store_result(
                path, ok=False, dest=dest, error=str(e), manual=manual
            )
        except Exception:
            point = {"ok": False, "error": str(e), "manual": manual}
        log("traceroute:", e)
        return point
